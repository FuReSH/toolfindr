...
export const TitleBox = ({ title, subtitle_long, description, logo_title }) => {
  ...
  const { 
    ...
    images } = useSiteMetadata()  // Anpassung an die neue Struktur
  ...
  const filename = logo_title || images.logo_title  // Zugriff auf logo_title im images-Objekt
  ...
  if (!imageNode) {
    return <div>Could not find the image: {filename}</div>
  }
